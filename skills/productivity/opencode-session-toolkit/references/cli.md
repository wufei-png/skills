...
`export --sanitize` calls the installed OpenCode CLI once per selected session as `opencode export SESSION_ID --sanitize` and stores the native JSON output without rewriting or locally sanitizing it. It uses the database resolved by `opencode db path`; `--db-path`, `--format jsonl`, and `--include-sensitive` are rejected. Native stdout is captured directly to a temporary file and must be one valid JSON object; a nonzero exit, timeout, status prefix, truncation, or invalid JSON aborts the whole batch and leaves no new output. Each native invocation is limited to 120 seconds. `--dry-run` still invokes the native CLI because unchanged and conflict status need the native payload; it does not write files.

All export files are staged inside the output directory and published as one batch, so publish stays on the destination filesystem. A normal publish failure rolls already-published files back and removes empty directories created for the batch; existing files are still preflighted, and `--overwrite` remains required for changed output.
